...
void ParseCmdArgs( int argc, const char* argv[] )
{
	if( argc < 5 )
	{
		printf("<pgmName> <texfilename> <subpixel> <edgeThreshold> <edgeThresholdMin>\n");
		exit(EXIT_FAILURE);
	}

    size_t strLen = strlen(argv[1]);
    memcpy(argTexfile, argv[1], strLen);
	argSubPixel = atof(argv[2]);
	argEdgeThreshold = atof(argv[3]);
	argEdgeThresholdMin = atof(argv[4]);
}
...
int main( int argc, const char* argv[] )
{
    ...
    sprintf(fullFileName, "%s-Tex-%f-%f-%f%s", file, argSubPixel, argEdgeThreshold, argEdgeThresholdMin, extension);
	WritePixelsToFile(
		fullFileName,
		surfaceWidth, surfaceHeight,
		4,
