...
static void recordGlCommands( struct eglContext* pEglContext, uint32_t time )
{
	...
	float fred = ( ( rand() % ( 255 - 1 + 1 ) ) + 1 );
	float fgreen = ( ( rand() % ( 255 - 1 + 1 ) ) + 1 );
	float fblue = ( ( rand() % ( 255 - 1 + 1 ) ) + 1 );
	float falpha = ( ( rand() % ( 255 - 0 + 1 ) ) + 0 );

	float red = (fred) / 255.0;
	float green = ( fgreen ) / 255.0;
	float blue = (fblue) / 255.0;
	float alpha = 1.0;	
#else
	float red = 0.0;
	float green = 0.0;
...
static void CaptureAndDump()
{
	...
	sprintf(fullFileName, "EglClientDump%s", extension);
	...
	pixelDumpSizeInBytes = surfaceWidth * surfaceHeight * 4;
